...
router.get('/', requireAuth, (req, res) => {
  ...
  const characters = db.prepare(
    'SELECT id, name, class, gold_gp, gold_sp, gold_cp FROM characters WHERE user_id = ?'
  ).all(req.user.id);
  res.json(characters);
});
...
router.post('/', requireAuth, (req, res) => {
  ...
  const result = db.prepare(
    'INSERT INTO characters (user_id, name, class, gold_gp, gold_sp, gold_cp) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(req.user.id, name.trim(), charClass.trim(), gold_gp, gold_sp, gold_cp);
