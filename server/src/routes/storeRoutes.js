...
router.get('/', requireAuth, (req, res) => {
  ...
  const stores = isDM
    ? db.prepare('SELECT * FROM stores ORDER BY name').all()
    : db.prepare('SELECT * FROM stores WHERE is_open = 1 ORDER BY name').all();
  res.json(stores);
});
...
router.post('/', requireDM, (req, res) => {
  ...
  ).run(name.trim(), description || null, location || null);
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  const multiplierRow = db.prepare("SELECT value FROM dm_settings WHERE key = 'price_multiplier'").get();
...
router.delete('/:id', requireDM, (req, res) => {
  const db = getDb();
  const store = db.prepare('SELECT id FROM stores WHERE id = ?').get(req.params.id);
  if (!store) return res.status(404).json({ error: 'Store not found' });
  db.prepare('DELETE FROM stores WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});

router.post('/:id/listings', requireDM, (req, res) => {
